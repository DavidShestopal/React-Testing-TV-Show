...
test('episodes list when the app is rendered', async () => {
  const { rerender, queryAllByTestId } = render(<Episodes episodes={[]} />);
  expect(queryAllByTestId(/episodeTester/i)).toStrictEqual([]);
  expect(queryAllByTestId(/episodeDiv/i)).toHaveLength(1);

  rerender(<Episodes episodes={episodes} />);
  expect(queryAllByTestId(/episodeTester/i)).toHaveLength(2);
});
